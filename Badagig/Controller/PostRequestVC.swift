...
//  badagig
//
//  Created by Perfect Aduh on 11/15/17.
...
class PostRequestVC: UIViewController {
    ...
    var passedSubCategory: SubCategory?
    ...
    override func viewDidLoad() {
        ...
        if let passedCategory = passedCategory {
            print("Category \(passedCategory.CategoryTItle)")
            categoryTxt.text = passedCategory.CategoryTItle
        }
        
        if let passedSubCategory = passedSubCategory {
            print("SubCategory \(passedSubCategory.subCategoryTItle)")
            subCategoryTxt.text = passedSubCategory.subCategoryTItle
        }
    ...
    @IBAction func submiteRequestButtonPressed(_ sender: Any) {
        ...
        guard let description = addDescriptionTextView.text, description != ""
            else {OperationQueue.main.addOperation {
                AlertErrorHandler.instance.showAlert(title: "Error", message: "Please enter a description")
                }
            return}
...
extension PostRequestVC: UIPickerViewDelegate, UIPickerViewDataSource {
